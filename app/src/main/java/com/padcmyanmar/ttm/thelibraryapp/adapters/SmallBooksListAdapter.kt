...
import com.padcmyanmar.ttm.thelibraryapp.viewholders.SmallBooksListViewHolder


class SmallBooksListAdapter (
    var bookItemDelegate: BookItemDelegate,
    var checkAudioOrEbooksFlagParam: Boolean) : RecyclerView.Adapter<SmallBooksListViewHolder>(){

    private var checkAudioOrEbooksFlag:Boolean = false

    private var mBooksListVoList: List<BooksListVO> = arrayListOf()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SmallBooksListViewHolder {
        val view =
            LayoutInflater.from(parent.context).inflate(
                R.layout.view_holder_book,
                parent,false)

        return SmallBooksListViewHolder(bookItemDelegate = bookItemDelegate,
            view)
    }

    override fun onBindViewHolder(holder: SmallBooksListViewHolder, position: Int) {
